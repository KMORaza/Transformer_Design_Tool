from PyQt5.QtWidgets import QDialog, QWidget, QHBoxLayout, QVBoxLayout, QFormLayout, QLabel, QLineEdit, QPushButton, QTextEdit, QComboBox, QSizePolicy, QCheckBox
...
class ThermalSimulationFEMWindow(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Transient Thermal Simulation (FEM)")
        ...
        self.simulation_time = QLineEdit("3600")  # Total simulation time (s)
        self.simulation_time.setFont(QFont("Consolas", 10))
        self.simulation_time.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        form_layout.addRow(QLabel("Simulation Time (s):"), self.simulation_time)
        ...
        output_label = QLabel("Transient Thermal Simulation Results")
        ...
        self.thermal_results = None  # Store simulation results
    ...
    def run_simulation(self):
        try:
            ...
            t_max = float(self.simulation_time.text())
            ...
            if convection_coeff <= 0 or airflow_velocity <= 0:
                ...
                return
            if t_max <= 0:
                self.output_text.setText("Error: Simulation time must be positive.")
                return
            ...
            # Run transient FEM simulation
            self.thermal_results = self._run_fem_simulation(
                core_loss=core_loss,
                ...
                ambient_temp=ambient_temp + 273.15,  # Kelvin
                airflow_velocity=airflow_velocity,
                operating_temp=operating_temp,
                t_max=t_max
            )
            
            # Update output (final time step)
            max_temp_c = self.thermal_results['max_temperature'][-1] - 273.15
            avg_temp_c = np.mean(self.thermal_results['temperature'][-1]) - 273.15
            conduction_loss = self.thermal_results['conduction_loss']
            convection_loss = self.thermal_results['convection_loss']
            radiation_loss = self.thermal_results['radiation_loss']
            hotspot_x, hotspot_y = self.thermal_results['hotspot_position'][-1]
            ...
            output += f"Simulation Time: {t_max:.0f} s\n"
            ...
            output += "Animation running..."
            self.output_text.setText(output)
            
            # Start animation
            self.plot_widget.animate_plot(
                self.thermal_results['x'],
                self.thermal_results['y'],
                self.thermal_results['temperature'],
                self.thermal_results['hotspot_position'],
                self.thermal_results['heat_flux'],
                self.thermal_results['times'],
                t_max,
                show_heat_flux
            )
    ...
    def _run_fem_simulation(self, core_loss, copper_loss, core_area, core_thermal_k, winding_thermal_k, emissivity, convection_coeff, ambient_temp, airflow_velocity, operating_temp, t_max):
        # 2D domain: core (square) with windings (annulus)
        core_side = math.sqrt(core_area)
        winding_thickness = 0.01  # 10 mm
        ...
        # Material properties
        thermal_k = np.ones(num_nodes) * core_thermal_k  # W/m·K
        density = np.ones(num_nodes) * 7650  # kg/m³ (silicon steel)
        specific_heat = np.ones(num_nodes) * 450  # J/kg·K (silicon steel)
        ...
        thermal_k[winding_region] = winding_thermal_k  # W/m·K (copper)
        density[winding_region] = 8960  # kg/m³ (copper)
        specific_heat[winding_region] = 385  # J/kg·K (copper)
        
        # Heat sources (W/m³)
        heat_source = np.zeros(num_nodes)
        core_volume = core_area * 0.1  # 0.1 m depth
        ...
        # FEM system: [C]*dT/dt + [K]*T = F
        K = sp.lil_matrix((num_nodes, num_nodes))  # Stiffness
        C = sp.lil_matrix((num_nodes, num_nodes))  # Capacitance
        F = np.zeros(num_nodes)  # Heat source
        ...
        for tri in triangles:
            ...
            cp_elem = np.mean(specific_heat[[n1, n2, n3]])
            Ke = k_elem * area * (np.outer(b, b) + np.outer(c, c))
            Ce = rho_elem * cp_elem * area / 3 * np.eye(3)
            for i in range(3):
                for j in range(3):
                    K[n1+i, n1+j] += Ke[i, j]
                    C[n1+i, n1+j] += Ce[i, j]
        ...
        save_interval = max(1, num_steps // 50)  # Save ~50 frames
        T = np.ones(num_nodes) * ambient_temp  # Initial temp (Kelvin)
        temperatures = []
        hotspots = []
        times = []
        ...
        for step in range(num_steps):
            ...
            if step % save_interval == 0:
                ...
                hotspots.append(nodes[max_temp_idx])
                times.append(step * dt)
        
        # Final step
        temperatures.append(T)
        hotspots.append(nodes[np.argmax(T)])
        times.append(t_max)
        
        # Heat fluxes (final step)
        ...
        return {
            'x': X,
            'y': Y,
            'temperature': [t.reshape(X.shape) for t in temperatures],
            'max_temperature': [np.max(t) for t in temperatures],
            'hotspot_position': hotspots,
...
class ThermalPlot(QWidget):
    ...
    def animate_plot(self, x, y, temperatures, hotspot_positions, heat_flux, times, t_max, show_heat_flux):
        try:
            self.ax.clear()
            T_celsius = temperatures[0] - 273.15
            contour = self.ax.contourf(x, y, T_celsius, levels=50, cmap='hot')
            cbar = self.figure.colorbar(contour, ax=self.ax, label='Temperature (°C)')
            hotspot, = self.ax.plot(
                hotspot_positions[0][0], hotspot_positions[0][1], 'r*',
                markersize=15, label=f'Hotspot: {np.max(T_celsius):.1f}°C'
            )
            time_text = self.ax.text(
                0.02, 0.98, f'Time: {times[0]:.0f} s',
            ...
            if show_heat_flux:
                ...
                quiver = self.ax.quiver(x, y, U, V, color='blue', scale=1e3)
            ...
            def update(frame):
                nonlocal contour
                T_celsius = temperatures[frame] - 273.15
                for c in contour.collections:
                    c.remove()
                contour = self.ax.contourf(x, y, T_celsius, levels=50, cmap='hot')
                hotspot.set_xdata([hotspot_positions[frame][0]])
                hotspot.set_ydata([hotspot_positions[frame][1]])
                hotspot.set_label(f'Hotspot: {np.max(T_celsius):.1f}°C')
                self.ax.legend()
                time_text.set_text(f'Time: {times[frame]:.0f} s')
                return contour.collections + [hotspot, time_text] + ([quiver] if quiver else [])
            
            self.anim = animation.FuncAnimation(
                self.figure, update, frames=len(temperatures), interval=50, blit=False
            )
