...
class LeakageAndStrayFieldEstWindow(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Leakage & Stray Field Estimation")
        ...
        self.calculate_btn.clicked.connect(self.calculate_leakage_stray)
        ...
        # Plot panel
        self.plot_widget = StrayFieldPlot()
        ...
        output_label = QLabel("Leakage & Stray Field Results")
    ...
    def calculate_leakage_stray(self):
        try:
            ...
            results = self.transformer.calculate(transformer_type, core_type, Vp, [Vs], freq, [power], eff, winding_config=winding_config)
            ...
            output += f"Peak Stray Field: {max(results['stray_fields']):.2f} A/m\n"
            output += f"Safe Distance (H < 0.1 A/m): {results['safe_distance']:.3f} m\n"
            self.output_text.setText(output)
            
            # Update plot
            self.plot_widget.update_plot(results['stray_fields'], results['distances'])
...
class StrayFieldPlot(QWidget):
    def __init__(self, parent=None):
        ...
        self.ax.set_title("Stray Magnetic Field")
        self.ax.set_xlabel("Distance from Core (m)")
        self.ax.set_ylabel("Magnetic Field Strength H (A/m)")
        self.ax.grid(True)
        
    def update_plot(self, stray_fields, distances):
        try:
            self.ax.clear()
            self.ax.plot(distances, stray_fields, label="Stray Field")
            self.ax.axhline(y=0.1, color='r', linestyle='--', label="Safe Threshold (0.1 A/m)")
            self.ax.set_xlim(0, max(distances))
            self.ax.set_ylim(0, max(stray_fields) * 1.1)
            self.ax.set_title("Stray Magnetic Field")
            self.ax.set_xlabel("Distance from Core (m)")
            self.ax.set_ylabel("Magnetic Field Strength H (A/m)")
            self.ax.grid(True)
