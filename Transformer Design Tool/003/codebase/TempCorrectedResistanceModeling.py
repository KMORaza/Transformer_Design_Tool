...
class TempCorrectedResistanceModelingWindow(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Temperature-Corrected Resistance Modeling")
        ...
        self.calculate_btn.clicked.connect(self.calculate_resistance)
        ...
        # Plot panel
        self.plot_widget = ResistancePlot()
        ...
        output_label = QLabel("Resistance Modeling Results")
    ...
    def calculate_resistance(self):
        try:
            ...
            output += f"Primary Resistance (Ambient, {ambient_temp}°C): {results['R_primary_ambient']*1000:.2f} mΩ\n"
            output += f"Primary Resistance (Operating, {operating_temp}°C): {results['R_primary_operating']*1000:.2f} mΩ\n"
            output += f"Secondary Resistance (Ambient, {ambient_temp}°C): {results['R_secondary_ambient'][0]*1000:.2f} mΩ\n"
            output += f"Secondary Resistance (Operating, {operating_temp}°C): {results['R_secondary_operating'][0]*1000:.2f} mΩ\n"
            output += f"Primary Copper Loss (Ambient): {results['copper_loss_primary_ambient']:.2f} W\n"
            output += f"Primary Copper Loss (Operating): {results['copper_loss_primary']:.2f} W\n"
            output += f"Secondary Copper Loss (Ambient): {results['copper_loss_secondary_ambient']:.2f} W\n"
            output += f"Secondary Copper Loss (Operating): {results['copper_loss_secondary']:.2f} W\n"
            output += f"Total Copper Loss (Ambient): {results['total_copper_loss_ambient']:.2f} W\n"
            output += f"Total Copper Loss (Operating): {results['total_copper_loss']:.2f} W\n"
            self.output_text.setText(output)
            
            # Update plot
            self.plot_widget.update_plot(
                results['copper_loss_primary_ambient'], results['copper_loss_primary'],
                results['copper_loss_secondary_ambient'], results['copper_loss_secondary']
            )
...
class ResistancePlot(QWidget):
    def __init__(self, parent=None):
        ...
        self.ax.set_title("Copper Losses at Different Temperatures")
        self.ax.set_ylabel("Power Loss (W)")
        self.ax.grid(True)
        
    def update_plot(self, primary_loss_ambient, primary_loss_operating, secondary_loss_ambient, secondary_loss_operating):
        try:
            self.ax.clear()
            categories = ['Primary (Ambient)', 'Primary (Operating)', 'Secondary (Ambient)', 'Secondary (Operating)']
            losses = [primary_loss_ambient, primary_loss_operating, secondary_loss_ambient, secondary_loss_operating]
            self.ax.bar(categories, losses, color=['#FF6666', '#FF9999', '#66B2FF', '#99CCFF'])
            self.ax.set_title("Copper Losses at Different Temperatures")
            self.ax.set_ylabel("Power Loss (W)")
            self.ax.grid(True)
