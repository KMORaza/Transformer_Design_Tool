...
class CoreSaturationAnalyzerWindow(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Core Saturation Analysis")
        ...
        self.calculate_btn.clicked.connect(self.calculate_saturation)
        ...
        # Plot panel
        self.plot_widget = SaturationPlot()
        ...
        output_label = QLabel("Saturation Analysis Results")
    ...
    def calculate_saturation(self):
        try:
            ...
            results = self.transformer.calculate(transformer_type, core_type, Vp, [Vs], freq, [power], eff, core_material)
            
            # Analyze saturation
            B_peak = results['max_flux_density']
            H_peak = results['H_peak']
            B_sat = results['saturation_flux_density']
            safety_margin = (B_sat - B_peak) / B_sat * 100
            saturation_status = "Saturated" if B_peak >= B_sat else "Not Saturated"
            ...
            output += f"Peak Flux Density: {B_peak:.2f} T\n"
            output += f"Saturation Flux Density: {B_sat:.2f} T\n"
            output += f"Peak Magnetic Field: {H_peak:.2f} A/m\n"
            output += f"Saturation Status: {saturation_status}\n"
            output += f"Safety Margin: {safety_margin:.2f}%\n"
            self.output_text.setText(output)
            
            # Update plot
            self.plot_widget.update_plot(results['flux_densities'][0], results['H_field'], B_sat, B_peak, H_peak)
...
class SaturationPlot(QWidget):
    def __init__(self, parent=None):
        ...
        self.ax.set_title("B-H Curve")
        self.ax.set_xlabel("Magnetic Field Strength H (A/m)")
        self.ax.set_ylabel("Magnetic Flux Density B (T)")
        self.ax.grid(True)
        
    def update_plot(self, flux_density, H_field, B_sat, B_peak, H_peak):
        try:
            self.ax.clear()
            # Plot B-H curve
            self.ax.plot(H_field, flux_density, label="B-H Curve")
            # Plot saturation line
            self.ax.axhline(y=B_sat, color='r', linestyle='--', label=f"Saturation (B={B_sat:.2f} T)")
            # Plot operating point
            self.ax.plot(H_peak, B_peak, 'go', label=f"Operating Point (B={B_peak:.2f} T, H={H_peak:.2f} A/m)")
            self.ax.set_xlim(-max(abs(min(H_field)), abs(max(H_field))) * 1.1,
                            max(abs(min(H_field)), abs(max(H_field))) * 1.1)
